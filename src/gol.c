...
static int to_int(int *num, const char *s) {
  errno = 0;
...
static void print_usage(const char argv0[]) {
  fprintf(
      ...
      "\t<infilename>:      file from which to load initial board "
      "configuration\n"
...
static void copy_board(char outboard[], const char inboard[], const int nrows,
                       const int ncols) {
...
static int boards_equalp(const char b1[], const char b2[], const int nrows,
                         const int ncols) {
  int i;
...
int main(int argc, char *argv[]) {
  ...
  if (verifyp) {
    ...
    /* Ping-pong between checkboard (contains the initial state) and
       inboard */
    final_board =
        sequential_game_of_life(inboard, checkboard, nrows, ncols, gens_max);
